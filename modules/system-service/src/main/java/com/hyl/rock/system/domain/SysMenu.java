...
public class SysMenu extends BaseEmptyEntity {
    ...
    /** 是否为外链（0是 1否） */
    @ApiModelProperty(name = "是否为外链 （0是 1否）")
    @TableField("is_frame")
    private String isFrame;

    /** 是否缓存（0缓存 1不缓存） */
    @ApiModelProperty(name = "是否缓存（0缓存 1不缓存）")
    @TableField("is_cache")
    ...
    /** 显示状态（0显示 1隐藏） */
    @ApiModelProperty(name = "显示状态（0显示 1隐藏）")
    @TableField("visible")
    private String visible;

    /** 菜单状态（0正常 1停用） */
    @ApiModelProperty(name = "菜单状态（0正常 1停用）")
    @TableField("status")
    private String status;

    /** 权限字符串 */
    @Size(min = 0, max = 100, message = "权限标识长度不能超过100个字符")
    @ApiModelProperty(name = "权限字符串")
    @TableField("perms")
